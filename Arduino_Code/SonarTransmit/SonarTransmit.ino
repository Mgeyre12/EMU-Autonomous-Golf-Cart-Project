  ...
  if(mySerial.available() > 0){
    ...
    if(mySerial.read() == 0xff){    //Judge packet header
      ...
      if(buffer_RTT[3] == CS) {
       ...
       // Serial.print("Distance:"); These needed to be commented out since it was interfering with sending the commands to Python 
       // Serial.print(Distance);
       // Serial.println("mm");
        
        if (Distance < 2280) {  // suppose to be around 5ft which would be approx. 1524mm
          Serial.println("A"); // Stop

        }
        else if ((2280 <= Distance) && (Distance <= 4580)) {  // suppose to be between 5ft and 15ft which would be approx. 1524mm to 4572mm 
          Serial.println("B"); // Slow down 

        }
        //else if ((4580 < Distance) && (Distance < 5000)) {  
        //  Serial.println("C");   //Left this in incase we need to and another section 
        //}
        else {
          Serial.println("D"); // Continue 
